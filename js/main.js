...
const arrOut = (arr,outInner) => {
  outInner.innerHTML = ``;
  for (let key of arr) {
    outInner.innerHTML += `<div class="item" >${key}<div class="item-btns flex"><div class="update flex-c"></div><div class="completed flex-c"></div> <div class="clear flex-c"></div></div></div>`;
  }
...
const itemClick = () => {
  ...
  items.forEach((elem, index) => {
    ...
    itemUpdate.addEventListener("click", () => {
        ...
        elem.outerHTML = `<div class="item item--update"><input class="item__input" value="${elem.textContent}"> <div class="item-btns flex"><div class="accept  flex-c">1</div></div>`
...
const tabsToggleAnimation = (tab) => {
  let wrapperInfo = document.querySelector('.to-wrapper').getBoundingClientRect()
  let tabInfo = tab.getBoundingClientRect()
  toggleItem.style.transform = `translateX(${tabInfo.left - wrapperInfo.left - 30}px)`
